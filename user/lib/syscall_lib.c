...
int syscall_declare_env_var(char *name, char *val, int is_export, int is_readonly) {
	return msyscall(SYS_declare_env_var, name, val, is_export, is_readonly);
}

int syscall_unset_env_var(char *name)
{
	return msyscall(SYS_unset_env_var, name);
}

int syscall_list_env_var(char *output_buf)
{
	return msyscall(SYS_list_env_var, output_buf);
}

int syscall_find_env_var(char *name, char *res_val)
{
	return msyscall(SYS_find_env_var, name, res_val);
}

int syscall_sync_penv_var()
{
	return msyscall(SYS_sync_penv_var);
}

int syscall_list_env_var1(char **name, char **res, int *cnt)
{
	return msyscall(SYS_list_env_var1, name, res, cnt);
}