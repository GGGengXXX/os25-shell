...
int elf_load_seg(Elf32_Phdr *ph, const void *bin, elf_mapper_t map_page, void *data) {
	u_long va = ph->p_vaddr;// Segment 的虚拟地址
	size_t bin_size = ph->p_filesz; // 文件中 Segment 的大小
	size_t sgsize = ph->p_memsz; // Segment 占用内存大小
	u_int perm = PTE_V;
	if (ph->p_flags & PF_W) {
		perm |= PTE_D;
	}
