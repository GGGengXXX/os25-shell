...
void write_directory(struct File *dirf, char *path) {
	...
	/*
	 * 以 传入参数 dirf 为根目录 
	 * dirf指向了 1024 个disk 每个disk可以容纳 16个 struct File*
	 * 找到一个空的 struct File* 并返回
	 */
	struct File *pdir = create_file(dirf);
	/*
	 * basename是 linux 常用库函数
	 * 包含在 头文件 libgen.h 中
	 * 在路径中提取文件名
	 */
